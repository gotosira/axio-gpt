...
export function Sidebar({ onNew, onSelect, currentId }: { onNew: () => void; onSelect: (id: string) => void; currentId?: string }) {
  ...
  return (
    <aside className="hidden md:flex w-72 shrink-0 flex-col gap-3 border-r p-3">
      <div className="text-xs uppercase tracking-wide opacity-60">{t('conversations') || 'Conversations'}</div>
      <button onClick={onNew} className="text-left rounded-md border px-3 py-2 text-sm hover:bg-gray-50">{t('newChat') || 'New chat'}</button>
      <div className="flex-1 overflow-auto flex flex-col gap-1">
        {list.length === 0 && <div className="text-xs opacity-60">{t('noConversations') || 'No conversations'}</div>}
        {list.map((c) => (
